...
static void renderTeamName(ClientGame *clientGame, S32 index, bool isWinningTeam, S32 left, S32 right, S32 top)
{
   ...
   RenderUtils::drawString (left  + 40,  top + 2, teamFontSize, clientGame->getTeamName(index).getString());
   RenderUtils::drawStringf(right - 140, top + 2, teamFontSize, "%d", ((Team *)(clientGame->getTeam(index)))->getScore());

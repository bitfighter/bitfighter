...
void EditorUserInterface::render()
{
   ...
   // Render any items generated by the levelgen script
   for(S32 i = 0; i < mLevelGenItems.size(); i++)
      renderItem(mLevelGenItems[i], false, false, true);
   ...
   // Render dock items
   for(S32 i = 0; i < mDockItems.size(); i++)
      renderItem(mDockItems[i], false, true, false);
...
void EditorUserInterface::renderPoly(Vector<Point> verts, bool isDockItem)
{
   glBegin(GL_LINE_STRIP);
      ...
      {
         Point v = isDockItem ? verts[j] : convertLevelToCanvasCoord(verts[j]);
         glVertex2f(v.x, v.y);
      }
   glEnd();
}

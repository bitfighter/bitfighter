...
static void renderMessageDoodads(const ClientGame *game, HelpItem helpItem, S32 textLeft, S32 textTop, S32 textBottom, const Color &color, F32 alpha)
{
   ...
   else if(helpItem == GameTypeAndTimer)
   {
      ...
      const S32 margin = TimeLeftRenderer::TimeLeftIndicatorMargin;
      const S32 x            = DisplayManager::getScreenInfo()->getGameCanvasWidth()  - margin - w;
      const S32 indicatorTop = DisplayManager::getScreenInfo()->getGameCanvasHeight() - margin - h - indicatorVerticalGap;

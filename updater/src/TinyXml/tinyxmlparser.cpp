	   ...
	   )
	{
		*value = 0;
		...
		if ( *(p+4) == ';' )
		{
			...
			if ( Zap::isAlpha(*(p+3)) ) *value += ( static_cast<char>(Zap::toLower(*(p+3))) - 'a' + 10 );
			else				     *value += ( *(p+3) - '0' );
		...
		{
			// two value entity
			if (Zap::isAlpha(*(p + 3))) *value += (static_cast<char>(Zap::toLower(*(p + 3))) - 'a' + 10) * 16;
			else				     *value += ( *(p+3) - '0' ) * 16;

			if (Zap::isAlpha(*(p + 4))) *value += (static_cast<char>(Zap::toLower(*(p + 4))) - 'a' + 10);
			else				     *value += ( *(p+4) - '0' );
							 ...
							 bool ignoreCase )
{
    ...
    if ( Zap::toLower(*p) == Zap::toLower(*tag) )
	{
		const char* q = p;
		...
		{
			while ( *q && *tag && Zap::toLower(*q) == Zap::toLower(*tag) )
			{
				++q;
				++tag;
			}

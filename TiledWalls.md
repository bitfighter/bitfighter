...
### Phase 4: Level files enable Fog of War
- Implement level-based control of Fog of War:
   - Create new FogOfWar level file entry in the header section
   - Valid lines are "FogOfWar Default", "FogOfWar Yes", and "FogOfWar No".  All are case insensitive.
   - If "FogOfWar Yes" is present, Fog of war is enabled
   - If "FogOfWar No" is present, then fog of war is disabled
   - If no FogOfWar entry is specified, or "FogOfWar Default" is specified, fog of war will be enabled for xtank vehicles, and disabled for bitfighter ships
   - The Game Parameters Menu in the level editor will have a new Fog of War section; users can toggle between "Yes", "No", and "Default"
   - At some point in the future, we might add some additional parameters to the FogOfWar line.
   - Client should be notified at the beginning of a game whether FOW is enabled or not.  This should be done with the other level metadata that we send.
   - Add test coverage:
     - Add testing for the FogOfWar level line parsing
     - Add testing to ensure FogOfWar value is loaded properly
     - Add testing to ensure FogOfWar value is saved properly
     - Add testing to ensure FogOfWar value survives round-trip through level editor
     - Anything else you think helpful
   - Modify LevelSpec.md to describe this new entry


### Phase 5: Fog-of-War Gating
- Integrate scope radius computation into tile enqueueing logic
- Test closest-first delivery
- Fog of war is disabled if the level header says Fog Of War No
